...
<head>
  <meta charset="UTF-8">
  <title>Cancelled Requests | RoadNova</title>
...
<h2 style="color:red;text-align:center">Cancelled Requests</h2>
...
cur = con.cursor()
cur.execute("""
  SELECT r.request_id, r.problem_type, r.refund_date, r.status,
         u.name, u.phone, u.email, u.address , r.refund , r.total_charge, r.cancelled_date
  FROM service_requests r
  JOIN users u ON r.user_id = u.user_id
  WHERE r.shop_id = %s AND r.status = 'Cancelled'
""", (shop_id,))
rows = cur.fetchall()

sn = 1
for req_id, prob, ref_date, status, name, phone, email, addr, refund, total, cancelled_date in rows:
    ...
    <tr>
      <td>{sn}</td>
      <td>{prob}</td>
      <td>{cancelled_date.strftime('%d-%m-%Y')}</td>
      <td>{name}</td>
      <td><button type="button" class="btn btn-warning btn-sm" data-bs-toggle="modal" data-bs-target="#refundModal{req_id}">
          View Details
        </button></td>
      <td>
  ...
  <!-- Refund Modal -->
  <div class="modal fade" id="refundModal{req_id}" tabindex="-1" aria-labelledby="refundModal{req_id}Label" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="refundModal{req_id}Label">Refund Details</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-body">
          <p><strong>Total Amount:</strong> ₹{total}</p>
          <p><strong>Refund Amount:</strong> ₹{refund if refund else 'N/A'}</p>
          <p><strong>Refund Status:</strong> 
            {"<span class='badge bg-success'>Paid</span>" if refund else "<span class='badge bg-secondary'>NO amount to refund Customer not paid service charge</span>"}
          </p>
          <p>{"<p><strong>Refund Date:</strong> " + ref_date.strftime('%d-%m-%Y') + "</p>" if refund else ""}
         </p>
        </div>
...
if not rows:
    print('<tr><td colspan="5" class="text-center">No Cancelled requests found.</td></tr>')
